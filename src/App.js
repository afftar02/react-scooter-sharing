...
function App() {
  const [isItemChosen, setIsItemChosen] = React.useState([]);
  ...
  React.useEffect(() => {
    setIsItemChosen(items.map((item) => (
      [item.scooterId, false]
    )))
  }, []);

  const onCardClick = (item) => {
    setIsItemChosen(prev => [...prev.filter(x => x[0] !== item.scooterId), [item.scooterId, true]]);//delete item with chosen scooterId by using filter and add this item with new value
  };

  const onDetailedCardCross = (item) => {
    setIsItemChosen(prev => [...prev.filter(x => x[0] !== item.scooterId), [item.scooterId, false]]);
  };
